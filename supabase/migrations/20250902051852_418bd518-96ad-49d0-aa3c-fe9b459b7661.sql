...
-- Fixed version without problematic GIN index
...
USING (
  ...
  -- Users can see events for deals they're involved in (using JSONB containment)
  payload @> jsonb_build_object('deal_id', (
    SELECT deals.id::text FROM deals 
...
-- Add performance indexes (using proper types)
